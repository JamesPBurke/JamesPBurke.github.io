---
permalink: /identity/
redirect_from:
  - /verify/
title: "🔐 Verify James Patrick Burke"
description: PGP key verification and signed statement for James Patrick Burke.
---
If you're a journalist, developer, or researcher who needs to contact me securely, or confirm this is really me — start here.

If I ever lose control of an account, this page and my signed statement are how you (or my friends) can tell.

---

## 🧾 Current PGP Key

- **Name:** James Patrick Burke
- **Email:** jp@JamesPBurke.com
- **Fingerprint:** `1C06 5792 7C3E E4DD 6638  C17D 016C D701 4FCD B4C9`
- **Expires:** July 2031
- [Download my public key](/verify/james-patrick-burke-public.asc)

---

## 🔗 Verified Accounts

Some accounts below are cryptographically linked to my PGP key, and independently checked by **[Keyoxide](https://keyoxide.org/1C0657927C3EE4DD6638C17D016CD7014FCDB4C9)** — green checks there mean the link is real, not just claimed. To check the entire list (even the ones unverifiable by Keyoxide) you can use PGP and my public key. [Instructions are here](/identity/instructions/).

### Cryptographically Verified via Keyoxide:

- **Keyoxide (identity aggregator):** [keyoxide.org/1C065792...](https://keyoxide.org/1C0657927C3EE4DD6638C17D016CD7014FCDB4C9)
- **Blog:** [https://jamespburke.com](https://jamespburke.com/blog/)
- **Bluesky:** [@jamespburke.com](https://bsky.app/profile/jamespburke.com)
- **GitHub:** [JamesPBurke](https://github.com/JamesPBurke)

### Other Social Profiles (Unverifiable by Keyoxide):

- **Instagram:** [drjpburke](https://www.instagram.com/drjpburke/)
- **LinkedIn:** [jpburke](https://www.linkedin.com/in/jpburke/)
- **Personal YouTube Channel:** [@JamesPBurke](https://www.youtube.com/@JamesPBurke)
- **Venmo:** [@DrJamesPBurke](https://account.venmo.com/u/DrJamesPBurke)
- **Keybase (obsolete):** [@sciuridae67](https://keybase.io/sciuridae67)
- **GMRS Callsign:** WSHY826
- **Signal:** If you know me personally, contact me on Signal via my phone number. If you don't know my phone number, connect with me via one of our other trusted channels to get my personal Signal account.

---

## 📜 Past Key Migration

I previously used a Keybase-managed key (fingerprint `f49117aa398befe31ee97c1dd9ea38d7db385371`), now revoked — Keybase is no longer awesome. [Signed migration statement.](/verify/migration.txt.asc)
